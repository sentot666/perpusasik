...
@section('title', __('Edit Anggota'))
@section('page-title', __('Edit Anggota'))
...
<div class="page-header">
    <h1 class="text-3xl font-bold text-slate-800 mb-1">{{ __('Edit Data Anggota') }}</h1>
    <p>{{ __('Perbarui informasi detail untuk anggota perpustakaan') }}</p>
</div>

<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="px-8 border-b border-slate-200 bg-slate-50 font-medium text-slate-700 py-4"><i class="bi bi-pencil-square text-indigo-600 mr-2"></i>{{ __('Form Edit Anggota') }}</div>
    <div class="p-8">
        ...
        <form method="POST" action="{{ route('members.update', $member) }}" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-wrap -mx-2 mb-6">
                        <div class="w-full md:w-1/2 px-4">
                            ...
                            <select name="identity_type" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none bg-white py-2 px-4">
                                <option value="KTP" {{ old('identity_type', $member->identity_type) == 'KTP' ? 'selected' : '' }}>{{ __('KTP') }}</option>
                                <option value="SIM" {{ old('identity_type', $member->identity_type) == 'SIM' ? 'selected' : '' }}>{{ __('SIM') }}</option>
                                <option value="Kartu Pelajar" {{ old('identity_type', $member->identity_type) == 'Kartu Pelajar' ? 'selected' : '' }}>{{ __('Kartu Pelajar') }}</option>
                                <option value="KTM" {{ old('identity_type', $member->identity_type) == 'KTM' ? 'selected' : '' }}>{{ __('KTM (Mahasiswa)') }}</option>
                            </select>
                        </div>
                        <div class="w-full md:w-1/2 px-4">
                    ...
                    <div class="flex flex-wrap -mx-2 mb-6">
                        <div class="w-full md:w-1/2 px-4">
                            ...
                            <select name="gender" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none bg-white py-2 px-4">
                                ...
                                <option value="L" {{ old('gender', $member->gender) == 'L' ? 'selected' : '' }}>{{ __('Laki-laki') }}</option>
                                <option value="P" {{ old('gender', $member->gender) == 'P' ? 'selected' : '' }}>{{ __('Perempuan') }}</option>
                            </select>
                        </div>
                        <div class="w-full md:w-1/2 px-4">
                            ...
                            <select name="member_type" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none bg-white py-2 px-4" required>
                                <option value="Siswa SD" {{ old('member_type', $member->member_type) == 'Siswa SD' ? 'selected' : '' }}>{{ __('Siswa SD') }}</option>
                                <option value="Siswa SMP" {{ old('member_type', $member->member_type) == 'Siswa SMP' ? 'selected' : '' }}>{{ __('Siswa SMP') }}</option>
                                <option value="Siswa SMA" {{ old('member_type', $member->member_type) == 'Siswa SMA' ? 'selected' : '' }}>{{ __('Siswa SMA') }}</option>
                                <option value="Guru" {{ old('member_type', $member->member_type) == 'Guru' ? 'selected' : '' }}>{{ __('Guru') }}</option>
                                <option value="Umum" {{ old('member_type', $member->member_type) == 'Umum' ? 'selected' : '' }}>{{ __('Umum') }}</option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="w-full md:w-1/2 px-4">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="form-switch flex items-center gap-2 mt-2">
                            ...
                            <label class="text-sm text-slate-700 text-slate-500" for="isActiveSwitch" style="font-size:0.8rem">{{ __('Aktifkan Anggota') }}</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="justify-end flex gap-2">
                <a href="{{ route('members.show', $member) }}" class="inline-flex items-center justify-center text-sm font-medium rounded-lg text-slate-700 border border-slate-200 border-slate-300 hover:bg-slate-50 transition-colors gap-2 py-2 px-6">{{ __('Batal') }}</a>
