...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-6 px-6">
    ...
    <a href="{{ route('opac.index') }}" class="inline-flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium rounded-lg text-slate-700 border border-slate-200 border-slate-300 hover:bg-slate-50 transition-colors mb-6 px-4">
        <i class="bi bi-arrow-left mr-1"></i>Kembali ke Pencarian
    </a>
    ...
    <div class="flex flex-wrap -mx-4">
        ...
        <div class="w-full lg:w-2/3 px-4">
                    ...
                    <div class="flex flex-wrap -mx-3">
                        ...
                        <div class="w-full md:w-2/3 px-4">
                            ...
                            <table class="border-0 text-sm w-full text-left text-slate-600 whitespace-nowrap [&>thead>tr>th]:px-4 [&>thead>tr>th]:py-3 [&>thead>tr>th]:bg-slate-50 [&>thead>tr>th]:font-semibold [&>thead>tr>th]:text-slate-700 [&>thead>tr>th]:border-b [&>thead>tr>th]:border-slate-200 [&>tbody>tr>td]:px-4 [&>tbody>tr>td]:py-3 [&>tbody>tr]:border-b [&>tbody>tr]:border-slate-100 [&>tbody>tr:last-child]:border-0 mb-0" style="font-size:0.875rem">
                                <tr>
                                    <td style="width:130px" class="text-slate-500">PENGARANG</td>
                                    <td>:
                                ...
                                <tr>
                                    <td class="text-slate-500">DESKRIPSI FISIK</td>
            ...
            {{-- Copies status table --}}
            <div class="shadow-sm border-0 mb-6 bg-white rounded-xl border border-slate-200 overflow-hidden" style="border-radius:12px">
                <div class="bg-white px-8 border-b border-slate-200 bg-slate-50 font-medium text-slate-700 font-bold py-4"><i class="bi bi-upc-scan text-indigo-600 mr-2"></i>Ketersediaan Eksemplar Fisik</div>
                <div class="p-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="inline-block px-2.5 py-0.5 rounded text-xs font-medium bg-success-subtle border border-slate-200 border-success-subtle text-emerald-600">Tersedia</span>
                                        @elseif($item->status === 'Dipinjam')
                                        <span class="inline-block px-2.5 py-0.5 rounded text-xs font-medium bg-warning-subtle border border-slate-200 border-warning-subtle text-amber-500 text-slate-800">Dipinjam (Jatuh tempo: {{ $item->activeCirculation?->due_date?->format('d/m/Y') ?? '-' }})</span>
                                        @else
                                ...
                                @empty
                                <tr><td colspan="5" class="text-center text-slate-500 py-4">Tidak ada data eksemplar fisik</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="w-full lg:w-1/3 px-4">
            <div class="shadow-sm border-0 bg-white rounded-xl border border-slate-200 overflow-hidden mb-6" style="border-radius:12px">
                <div class="bg-white px-8 border-b border-slate-200 bg-slate-50 font-medium text-slate-700 font-bold py-4"><i class="bi bi-bookmarks-fill text-amber-500 mr-2"></i>Koleksi Terkait</div>
                <div class="p-0">
                    <ul class="divide-y divide-slate-100 flex flex-col" style="border-radius:12px">
                        ...
                        @empty
                        <li class="border-0 block text-center text-slate-500 py-6">Belum ada koleksi sejenis</li>
                        @endforelse
                    </ul>
                </div>
            </div>
