...
<div x-data="{ showAddModal: false }">
    <div class="page-header justify-between items-start flex mb-6">
        <div>
            <h1 class="text-3xl font-bold text-slate-800 mb-1">{{ __('Buku Tamu & Aktivitas Harian') }}</h1>
            <p class="text-slate-500 mt-1">{{ __('Pendataan aktivitas harian tamu dan kunjungan institusi/kelompok di perpustakaan') }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('guest-books.export', request()->all()) }}" class="gap-1 inline-flex items-center justify-center text-sm font-medium rounded-lg btn-gradient-green text-white transition-colors flex gap-2 py-2 px-6 shadow-sm">
            ...
            <button type="button" @click="showAddModal = true" class="gap-1 items-center inline-flex justify-center text-sm font-medium rounded-lg btn-gradient-blue transition-colors text-white flex gap-2 py-2 px-6">
                <i class="bi bi-journal-plus text-base"></i>{{ __('Catat Kunjungan Baru') }}
            </button>
        </div>
    </div>
    ...
    <div class="px-8 py-2">
        <form method="GET" class="items-end flex flex-wrap -mx-2" id="guestBookFilterForm">
            <div class="w-full md:w-1/3 px-4">
                <label class="block text-sm font-medium text-slate-700 mb-1" style="font-size:0.75rem;font-weight:600">{{ __('CARI TAMU / INSTANSI') }}</label>
                <div class="flex w-full text-sm">
                    ...
                    <input type="text" name="search" class="border-l-0 bg-slate-50 w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" placeholder="{{ __('Cari nama, instansi, atau tujuan...') }}" value="{{ request('search') }}">
                </div>
            </div>
            ...
            <div class="w-auto px-4">
                ...
                <a href="{{ route('guest-books.index') }}" class="inline-flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium rounded-lg text-slate-700 border border-slate-200 border-slate-300 hover:bg-slate-50 transition-colors px-4"><i class="bi bi-x"></i> {{ __('Reset') }}</a>
            </div>
            <div class="ml-auto text-slate-500 w-auto px-4" style="font-size:0.8rem; padding-bottom: 5px;">
                {{ $activities->total() }} {{ __('kunjungan tercatat') }}
            </div>
        </form>
    </div>
</div>
            ...
            <table class="align-middle w-full text-left text-sm text-slate-600 whitespace-nowrap [&>thead>tr>th]:px-4 [&>thead>tr>th]:py-3 [&>thead>tr>th]:bg-slate-50 [&>thead>tr>th]:font-semibold [&>thead>tr>th]:text-slate-700 [&>thead>tr>th]:border-b [&>thead>tr>th]:border-slate-200 [&>tbody>tr>td]:px-4 [&>tbody>tr>td]:py-3 [&>tbody>tr]:border-b [&>tbody>tr]:border-slate-100 [&>tbody>tr:last-child]:border-0 [&>tbody>tr:hover]:bg-slate-50" style="font-size: 0.88rem;">
                <thead>
                    <tr class="bg-slate-50">
                        <th class="pl-4" style="width: 60px;">{{ __('No') }}</th>
                        ...
                        <th class="text-center" style="width: 100px;">{{ __('Aksi') }}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="pr-4 text-center">
                            <form action="{{ route('guest-books.destroy', $activity) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus catatan kunjungan ini?') }}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-600 bg-red-50 hover:bg-red-100 transition-colors" title="{{ __('Hapus catatan') }}">
                    ...
                    <tr>
                        <td colspan="8" class="py-8 text-center text-slate-500">
                            ...
                            {{ __('Belum ada catatan aktivitas kunjungan tamu untuk hari ini atau pencarian Anda.') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
{{-- Add Guest Modal --}}
    <div x-show="showAddModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6">
        <div @click.outside="showAddModal = false" class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden relative mt-10">
            <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center btn-gradient-blue text-white">
                <h5 class="font-semibold flex items-center gap-2">
                    <i class="bi bi-journal-plus text-lg font-medium"></i>{{ __('Catat Kunjungan Tamu') }}
                </h5>
                ...
                <div class="p-8 max-h-[70vh] overflow-y-auto">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full md:w-1/2 px-4">
                            <label for="visit_date" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Tanggal Kunjungan') }} <span class="text-red-600">*</span></label>
                        ...
                        <div class="w-full md:w-1/2 px-4">
                            <label for="visit_time" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Waktu Kunjungan') }} <span class="text-red-600">*</span></label>
                    ...
                    {{-- Input: Nama --}}
                    <div class="mb-6">
                        <label for="name" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Nama Lengkap Tamu / Perwakilan') }} <span class="text-red-600">*</span></label>
                        <div class="flex w-full">
                            ...
                            <input type="text" name="name" id="name" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" placeholder="{{ __('Ketik nama lengkap...') }}" required>
                        </div>
                    </div>

                    {{-- Input: Instansi / Asal --}}
                    <div class="mb-6">
                        <label for="institution" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Kunjungan Dari (Instansi / Kelas / Asal)') }} <span class="text-red-600">*</span></label>
                        <div class="flex w-full">
                            <span class="flex items-center px-3 py-2 bg-slate-100 border border-slate-300 text-slate-600"><i class="bi bi-building"></i></span>
                            <input type="text" name="institution" id="institution" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" placeholder="{{ __('Ketik instansi asal atau alamat...') }}" required>
                        </div>
                    </div>

                    {{-- Input: Tujuan --}}
                    <div class="mb-6">
                        <label for="purpose" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Tujuan Kunjungan') }} <span class="text-red-600">*</span></label>
                        <div class="flex w-full">
                            ...
                            <input type="text" name="purpose" id="purpose" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" placeholder="{{ __('Ketik tujuan kunjungan (studi banding, baca buku, dll.)...') }}" required>
                        </div>
                    </div>

                    {{-- Row: Jumlah Peserta --}}
                    <div class="mb-6">
                        <label for="participants_count" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Jumlah Peserta') }} <span class="text-red-600">*</span></label>
                        <div class="flex w-full">
                            ...
                            <span class="flex items-center px-3 py-2 bg-slate-100 border border-slate-300 text-slate-600">{{ __('Orang') }}</span>
                        </div>
                        <div class="form-text">{{ __('Masukkan jumlah orang yang ikut rombongan (termasuk perwakilan).') }}</div>
                    </div>

                    {{-- Input: Catatan --}}
                    <div class="mb-0">
                        <label for="notes" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Catatan Tambahan') }}</label>
                        <textarea name="notes" id="notes" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" rows="2" placeholder="{{ __('Opsional...') }}"></textarea>
                    </div>

                </div>
                <div class="px-8 py-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
                    <button type="button" @click="showAddModal = false" class="inline-flex items-center justify-center text-sm font-medium rounded-lg text-slate-700 border border-slate-200 border-slate-300 hover:bg-slate-50 transition-colors py-2 px-6">{{ __('Batal') }}</button>
                    <button type="submit" class="inline-flex items-center justify-center text-sm font-medium rounded-lg btn-gradient-blue transition-colors text-white font-semibold py-2 px-6">{{ __('Simpan Catatan') }}</button>
                </div>
            </form>
        </div>
    </div>
