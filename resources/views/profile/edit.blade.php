...
<div class="page-header">
    <h1 class="text-3xl font-bold text-slate-800 mb-1">{{ __('Pengaturan Akun') }}</h1>
    <p>{{ __('Perbarui data profil dan password akun Anda') }}</p>
</div>
...
<div class="flex flex-wrap -mx-3">
    ...
    <div class="w-full md:w-1/2 px-4">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
            <div class="px-8 border-b border-slate-200 bg-slate-50 font-medium text-slate-700 py-4"><i class="bi bi-person-fill-gear text-indigo-600 mr-2"></i>{{ __('Informasi Profil') }}</div>
            <div class="p-8">
                <form method="POST" action="{{ route('profile.update') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="form-text">{{ __('Username tidak dapat diubah') }}</div>
                    </div>
                    ...
                    <button type="submit" class="w-full inline-flex items-center justify-center text-sm font-medium rounded-lg btn-gradient-blue transition-colors text-white font-semibold gap-2 py-2 px-6">{{ __('Simpan Profil') }}</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="w-full md:w-1/2 px-4">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
            <div class="px-8 border-b border-slate-200 bg-slate-50 font-medium text-slate-700 py-4"><i class="bi bi-shield-lock-fill text-red-600 mr-2"></i>{{ __('Ganti Password') }}</div>
            <div class="p-8">
                <form method="POST" action="{{ route('profile.password') }}">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Password Saat Ini') }}</label>
                        <input type="password" name="current_password" class="@error('current_password') @enderror w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none border-red-500 focus:border-red-500 focus:ring-red-500 py-2 px-4" required placeholder="{{ __('Masukkan password sekarang') }}">
                        @error('current_password')<div class="text-xs text-red-500 mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Password Baru') }}</label>
                        <input type="password" name="password" class="@error('password') @enderror w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none border-red-500 focus:border-red-500 focus:ring-red-500 py-2 px-4" required placeholder="{{ __('Min. 8 karakter') }}">
                        @error('password')<div class="text-xs text-red-500 mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Konfirmasi Password Baru') }}</label>
                        <input type="password" name="password_confirmation" class="w-full rounded-lg border border-slate-200 border-slate-300 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none py-2 px-4" required placeholder="{{ __('Ulangi password baru') }}">
                    </div>

                    <button type="submit" class="w-full inline-flex items-center justify-center text-sm font-medium rounded-lg btn-gradient-red transition-colors text-white font-semibold gap-2 py-2 px-6">{{ __('Ubah Password') }}</button>
                </form>
            </div>
        </div>
    </div>
