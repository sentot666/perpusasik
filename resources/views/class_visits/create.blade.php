...
@section('title', __('Tambah Jadwal Kunjungan'))
@section('page-title', __('Tambah Jadwal Kunjungan'))
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-4xl mb-12">
    <div class="px-6 sm:px-8 border-b border-slate-200 bg-slate-50 font-semibold text-slate-700 py-4 flex items-center justify-between">
        <div class="flex items-center gap-2.5">
            ...
            <span>{{ __('Form Jadwal Kunjungan Kelas') }}</span>
        </div>
    ...
    <div class="p-6 sm:p-8">
        ...
        <form action="{{ route('class-visits.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-8">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="level" id="level" class="w-full rounded-lg border border-slate-300 py-2.5 px-3.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors bg-white appearance-none" required>
                            ...
                            <option value="sd" {{ old('level') == 'sd' ? 'selected' : '' }}>{{ __('Sekolah Dasar (SD)') }}</option>
                            <option value="smp" {{ old('level') == 'smp' ? 'selected' : '' }}>{{ __('Menengah Pertama (SMP)') }}</option>
                            <option value="sma" {{ old('level') == 'sma' ? 'selected' : '' }}>{{ __('Menengah Atas (SMA)') }}</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="day" id="day" value="{{ old('day') }}" class="w-full rounded-lg border border-slate-300 py-2.5 pl-3.5 pr-9 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="{{ __('Contoh: Senin / Selasa') }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" name="date" id="date" value="{{ old('date') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-3.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors bg-white">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="time" id="time" value="{{ old('time') }}" class="w-full rounded-lg border border-slate-300 py-2.5 pl-3.5 pr-9 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="{{ __('Contoh: 08:00 - 08:45 WIB') }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="class_name" id="class_name" value="{{ old('class_name') }}" class="w-full rounded-lg border border-slate-300 py-2.5 pl-3.5 pr-9 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="{{ __('Contoh: Kelas 1A / Kelas 7B / Kelas 10-IPA') }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="teacher_name" id="teacher_name" value="{{ old('teacher_name') }}" class="w-full rounded-lg border border-slate-300 py-2.5 pl-3.5 pr-9 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="{{ __('Contoh: Bpk. Budi Santoso, S.Pd') }}" required>
            ...
            <div class="flex justify-end items-center gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit" class="inline-flex items-center justify-center text-xs sm:text-sm font-semibold rounded-lg btn-gradient-blue shadow-md shadow-indigo-500/20 text-white py-2.5 px-6 transition-all transform hover:-translate-y-0.5 gap-2">
                    ...
                    <span>{{ __('Simpan Jadwal Kunjungan') }}</span>
                </button>
            </div>
        </form>
    </div>
