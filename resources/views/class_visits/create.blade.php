...
<div class="page-header mb-6">
    <h1 class="text-3xl font-bold text-slate-800 mb-1">Tambah Jadwal Kunjungan</h1>
    <p class="text-slate-500 text-sm">Tambahkan rutinitas kunjungan kelas baru ke perpustakaan.</p>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-12">
    <div class="px-6 sm:px-8 border-b border-slate-200 bg-slate-50 font-semibold text-slate-700 py-4 flex items-center">
        <i class="bi bi-calendar-plus text-indigo-600 mr-3 text-lg"></i> Form Jadwal Kunjungan
    </div>
    
    <div class="p-6 sm:p-8">
        <form action="{{ route('class-visits.store') }}" method="POST" class="max-w-4xl">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-8">
                <div>
                    ...
                    <select name="level" id="level" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" required>
                        ...
                        <option value="smp" {{ old('level') == 'smp' ? 'selected' : '' }}>Menengah Pertama (SMP)</option>
                        <option value="sma" {{ old('level') == 'sma' ? 'selected' : '' }}>Menengah Atas (SMA)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="day" id="day" value="{{ old('day') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="Contoh: Senin" required>
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors">
                ...
                <div>
                    ...
                    <input type="text" name="time" id="time" value="{{ old('time') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="Contoh: 08:00 - 08:45" required>
                ...
                <div>
                    ...
                    <input type="text" name="class_name" id="class_name" value="{{ old('class_name') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="Contoh: Kelas 1A" required>
                ...
                <div>
                    ...
                    <input type="text" name="teacher_name" id="teacher_name" value="{{ old('teacher_name') }}" class="w-full rounded-lg border border-slate-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="Contoh: Bpk. Budi Santoso" required>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit" class="inline-flex items-center justify-center text-sm font-semibold rounded-lg btn-gradient-blue shadow-md shadow-indigo-500/20 text-white py-2 px-6 transition-all transform hover:-translate-y-0.5">
                    <i class="bi bi-save mr-2"></i> Simpan Jadwal
                </button>
            </div>
        </form>
    </div>
